...
# Check events
kubectl get events -n dclaw-audit --sort-by='.lastTimestamp'

# Verify database connection
kubectl exec -n dclaw-audit deployment/dclaw-audit-backend --   python -c "import asyncio; from sqlalchemy import text; ..."
```
...
2. Check the connection string secret:
   ```bash
   kubectl get secret dclaw-audit-db-credentials -n dclaw-audit
   ```
