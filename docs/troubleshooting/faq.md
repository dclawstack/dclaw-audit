...
### How do I update DClaw Audit?

Update the `version` field in the DClawApp CRD. The operator will perform a rolling update.

```bash
kubectl patch dclawapp audit --type merge -p '{"spec":{"version":"0.2.0"}}'
```
...
### Can I run DClaw Audit without Kubernetes?

Yes, for local development:

```bash
cd dclaw-audit/frontend && npm run dev
cd dclaw-audit/backend && uvicorn dclaw_audit.main:app --reload
```
