...
export async function listEvidenceFiles(requestId: string) {
  return fetchJson<EvidenceFile[]>(`/api/v1/evidence-files?request_id=${requestId}`);
}

export async function uploadEvidenceFile(requestId: string, file: File, uploadedBy?: string, notes?: string) {
  ...
  const res = await fetch(`${base}/api/v1/evidence-files/upload`, { method: "POST", body: form });
  if (!res.ok) throw new ApiError(`Upload failed: ${res.status}`, res.status);
  return res.json() as Promise<EvidenceFile>;
}

export async function deleteEvidenceFile(id: string) {
  return fetchJson<void>(`/api/v1/evidence-files/${id}`, { method: "DELETE" });
}
...
export interface PrioritizeFindingsResponse {
  prioritized_findings: { finding_id: string; priority_rank: number; reason: string }[];
  critical_path: string[];
  summary: string;
}

export async function aiCheckEvidenceCompleteness(engagementId: string) {
  return fetchJson<EvidenceCompletenessResponse>("/api/v1/ai/check-evidence-completeness", {
    method: "POST",
    body: JSON.stringify({ engagement_id: engagementId }),
  });
}

export async function aiPrioritizeFindings(engagementId: string) {
  return fetchJson<PrioritizeFindingsResponse>("/api/v1/ai/prioritize-findings", {
    method: "POST",
    body: JSON.stringify({ engagement_id: engagementId }),
  });
}
